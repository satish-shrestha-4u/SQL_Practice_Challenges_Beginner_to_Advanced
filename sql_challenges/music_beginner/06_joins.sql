...
FROM
    artists AS a      -- tables can receive an alias
        INNER JOIN
...
SELECT 
    artist_name,   -- these columns don't have to be identified by table
    song_name,      -- but it does make things easy to read if someone reviews your code
	country
FROM
    artists AS a
        LEFT JOIN
...
SELECT 
    s.song_name, a.artist_name, r.rating, r.review, r.username
FROM
    ratings AS r
        LEFT JOIN
...
SELECT 
    a.id, a.artist_name, AVG(r.rating) AS average_rating
FROM
    artists AS a
        LEFT JOIN
    songs AS s ON a.id = s.artist_id
...
SELECT * FROM artists cross join songs;


-- Full Outer Join in MySQL

/* Part 1: All artists + Matching songs */
SELECT *
FROM artists AS a
LEFT JOIN songs AS s ON a.id = s.artist_id

UNION

/* Part 2: All songs + Matching artists */
SELECT *
FROM artists AS a
RIGHT JOIN songs AS s ON a.id = s.artist_id;